            ...
            <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-4 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <div className={`w-2.5 h-2.5 rounded-full ${statusData?.has_key ? 'bg-emerald-500' : 'bg-cyan-500'}`} />
                  <span className="font-semibold text-white text-sm">
                    {statusData?.has_key ? 'FortyGuard Live API Connected' : 'Calibrated Microclimate Model'}
                  </span>
                </div>
                <p className="text-xs text-slate-400 leading-relaxed font-mono">
                  Endpoint: https://api.fortyguard.com/v1
                  <br />
                  Key: {statusData?.masked_key || 'Environment Managed'}
                </p>
              </div>
